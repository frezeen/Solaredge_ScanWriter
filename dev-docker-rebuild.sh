...
if [[ ! -f ".env" ]]; then
    if [[ -f ".env.example" ]]; then
        ...
        echo ""
        echo -e "${YELLOW}Variabili OPZIONALI (Modbus real-time):${NC}"
        echo -e "${CYAN}REALTIME_MODBUS_HOST=192.168.1.100${NC} → IP del tuo inverter"
        echo -e "${CYAN}REALTIME_MODBUS_PORT=1502${NC} → Porta Modbus (default 1502)"
        echo ""
